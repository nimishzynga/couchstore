...
void handle_state(conn *c, int thread_id) {
    ...
    switch(c->state) {
        ...
        case task_read_socket:
            ...
            break;
        case task_local_query:
            dispatch_local_task(c);
            break;
...
#define URL "/default/_design/1/_view/1?stale=false&limit=10"

// take input as send request and return the result.
int query_start() {
    global_data.thread_num = 3;
    init_query();
    send_request sq[2] = {{.url = URL,
        .server = "127.0.0.1",
        .port = 9500,
...
int main() {
    int size;
    char *data = read_query_data(size);
    if (data == NULL) {
        return 1;
    }
    main_task_t *t = parse_query_args(data, size);
    FILE *fp = fopen("/tmp/logs", "w");
    fprintf(fp, "got data %s\n", t->sr->url);
    fclose(fp);
    //query_dispatch(t);
}


//create a main task with local and remote and dispatch it
//from it here
int query_dispatch(main_task_t *mt) {
    add_tasks(mt);
}
